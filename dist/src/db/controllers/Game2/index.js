...
const importBoxScores = async () => {
    ...
    for await (const game of models_1.Game2.findOne({
        ...
        if (boxScore && unpopulatedGame) {
            ...
            if (boxScore.home) {
                ...
                if (boxScore.home.stats) {
                    ...
                    /** Currently doesn't add periods due to type error
                     * periods isn't a mongoose subdocument in my schema, which
                     * is causing problems
                     */
                    if (periods) {
                        periods.map((p) => {
